...
function startExperiment(pid, cond, config, screens) {
  ...
  screens.showWelcome(cond.welcomeText, () => {
    ...
    const game = new Game(cond, config.players, config.throwCount, pid, {
      ...
      onGameEnd: (log) => {
        ...
        screens.showThankYou({
          ...
          onDownload: () => {
            if (!config.askIdOnGameEnd) triggerDownload();
          },
        });
        if (!config.askIdOnGameEnd) {
          // Entspricht dem Original: ohne ID-Abfrage wird der Log direkt
          // "gespeichert" (hier: zum Download bereitgestellt)
        }
      },
    });
