...
// Assets kaputtgeht.
const PLAYER_SPRITE_SIZE = 90; // Zielgröße auf dem Canvas (px)
...
export class Renderer {
  ...
  _drawPlayer(p, pos) {
    ...
    if (this.playerImg) {
      ...
      if (p.isHuman) {
        ctx.beginPath();
        ctx.arc(pos.x, pos.y, PLAYER_SPRITE_SIZE / 2 + 4, 0, Math.PI * 2);
        ctx.lineWidth = 3;
  ...
  draw() {
    ...
    // Dezenter "Spielfeld"-Kreis als Orientierungshilfe (rein optisch)
    const cx = this.canvas.width / 2;
    const cy = this.canvas.height / 2 + 10;
    const fieldR = Math.min(this.canvas.width, this.canvas.height) * 0.38;
    ctx.beginPath();
    ctx.ellipse(cx, cy, fieldR * 1.15, fieldR * 0.85, 0, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(74, 125, 252, 0.06)";
    ctx.fill();
